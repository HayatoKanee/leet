...
"""We can convert it into a shortest path problem, well we can either find all pair shortest or apply dijkstra for every element"""
...
class Solution:
    def minimumCost(self, source: str, target: str, original: List[str], changed: List[str], cost: List[int]) -> int:
        ...
        for i in range(n):
            if source[i] == target[i]:
                continue
            current_cost = self.dijkstra(source[i], target[i], graph)
            if current_cost == -1:
                return -1
            cost += current_cost
        return cost
    ...
    def dijkstra(self, source: str, target: str, graph: Dict[str, List[Tuple[str, int]]]) -> int:
        distance = {source: 0}
        queue = [(0, source)]
        heapq.heapify(queue)
        while queue:
